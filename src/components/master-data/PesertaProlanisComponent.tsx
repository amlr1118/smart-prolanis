...
export default function PesertaProlanisComponent() {
  ...
  const handleSelectChangeDiagnosa = (value: string) => {
    ...
    // (Opsional) Jika ada error diagnosa sebelumnya, hilangkan saat user memilih
    if (errors.diagnosa) {
      setErrors((prev) => ({ ...prev, diagnosa: [] }));
    }
  };

  // Ubah function untuk menerima parameter page
  const getPeserta = async (page: number = 1, search: string = "") => {
    setLoading(true);
    try {
      // Kirim parameter page dan search ke backend
      const response = await api.get(
        `/peserta-prolanis?page=${page}&search=${search}`,
      );
  ...
  useEffect(() => {
    getPeserta(currentPage, searchTerm);
  }, [currentPage]);
    ...
    try {
      ...
      if (editId) {
        response = await api.put(
          `/update-peserta-prolanis/${editId}`,
          formData,
        );
      } else {
        // Jika tidak ada, lakukan POST request untuk tambah baru
        response = await api.post("/tambah-peserta-prolanis", formData);
      }
      ...
      getPeserta(editId ? currentPage : 1, searchTerm);
  ...
  const handleDelete = async (id: number) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await api.delete(`/hapus-peserta-prolanis/${id}`);
        ...
        <div className="relative w-full p-4 overflow-y-auto bg-white rounded-3xl dark:bg-gray-900 lg:p-11">
          <div className="mb-6">
            <h4 className="text-2xl font-semibold text-gray-800 dark:text-white">
              {editId ? "Edit Data Peserta" : "Form Tambah Peserta Prolanis"}
            </h4>
          </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div>
                ...
                <Input
                  ...
                  value={formData.nama}
                  onChange={handleInputChange}
                  className={`mt-2 ${errors.nama ? "border-red-500" : ""}`}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={handleInputChange}
                  className={`mt-2 ${errors.no_hp ? "border-red-500" : ""}`}
